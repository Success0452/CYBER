...
import com.famous.cyber.databinding.ActivitySignUpBinding
...
class SignUp : AppCompatActivity() {
    ...
    private fun validateData() {
        //get data
        email = binding.emailSetup.text.toString().trim()
        password = binding.passwordSetup.toString().trim()

        //validate data
        if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()){
            //invalid email format
            binding.emailSetup.error = "Invalid Email Format"
        }else if (TextUtils.isEmpty(password)) {
            //password isn't entered
            binding.passwordSetup.error = "password isn't entered"

        }else if (password.length < 6){
            //password length is less than 6
            binding.passwordSetup.error = "Password must ave atleast 6 character"
        }else {
